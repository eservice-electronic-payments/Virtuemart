...
class plgVmPaymentEservice extends vmPSPlugin
{
    ...
    function plgVmConfirmedOrder($cart, $order)
    {
       ...
       // get the merchant account country 
        ...
        $payments = (new Payments())->environmentUrls([
            "merchantId" => $intelligentpaymentDetails['merchant_id'],
            "password" => $intelligentpaymentDetails['api_password'],
            "tokenURL" => $intelligentpaymentDetails['tokenUrl'],
            "paymentsURL" => $intelligentpaymentDetails['paymentUrl'],
            "baseUrl" => $intelligentpaymentDetails['cashierUrl'],
            "jsApiUrl" =>  $intelligentpaymentDetails['jsUrl'],
        ]);
        ...
        $merchantDecReqInd = 'N';
        $state = ShopFunctions::getStateByID($address->virtuemart_state_id, 'state_3_code');
...
//         $response_fields['virtuemart_paymentmethod_id'] = $intelligentpayment_data['pm'];
    ...
    function plgVmOnUpdateOrderPayment(&$order, $old_order_status)
    {
        ...
        if($payments_internal->xtype == 'AUTH_ONLY' && $old_order_status == $method->status_pending && $order->order_status == $method->status_success) {
            $captureAction = $payments->capture();
